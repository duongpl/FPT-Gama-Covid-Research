/***
* Name: ModelM33addDeath
* Author: DUONG
* Description: 
* Tags: Tag1, Tag2, TagN
***/

model ModelM33addDeath
...
global {
	...
	reflex end_simulation when: susceptible count (each.state = 3 and each.state = 4) = nb_of_people {
		do pause;
	}
...
species home {
	...
	action test {
		if (is_light and !is_workhour) {
			capture (susceptible inside self) where (each.end_point != nil and each.state != 4) as: people_in_home;
			do inf();
	...
	action inf {
		if (nb_total > 0 and (people_in_home count (each.state = 2) > 0)) {
			ask (people_in_home where (each.state != 2 and each.state != 3 and each.state != 1 and each.state != 4)) {
				if (have_mask) {
...
species industry {
	...
	action test {
		if (is_light and !is_workhour) {
			capture (susceptible inside self) where (each.end_point != nil and each.state != 4) as: people_in_industry;
			do inf();
		} else if (is_light and is_workhour) {
	...
	action inf {
		if (nb_total > 0 and (people_in_industry count (each.state = 2) > 0)) {
			ask (people_in_industry where (each.state != 2 and each.state != 3 and each.state != 1 and each.state != 4)) {
				if (have_mask) {
					if (flip(infected_rate * mask_rate)) {
						state <- 1;
						save_time <- nb_day;
...
species office {
	...
	action test {
		if (is_light and !is_workhour) {
			capture (susceptible inside self) where (each.end_point != nil and each.state != 4) as: people_in_office;
			do inf();
		} else if (is_light and is_workhour) {
	...
	action inf {
		if (nb_total > 0 and (people_in_office count (each.state = 2) > 0)) {
			ask (people_in_office where (each.state != 2 and each.state != 3 and each.state != 1 and each.state != 4)) {
				if (have_mask) {
					if (flip(infected_rate * mask_rate)) {
						state <- 1;
						save_time <- nb_day;
...
species park {
	...
	action test {
		if (is_light and !is_workhour) {
			capture (susceptible inside self) where (each.end_point != nil and each.state != 4) as: people_in_park;
			do inf();
		} else if (is_light and is_workhour) {
	...
	action inf {
		if (nb_total > 0 and (people_in_park count (each.state = 2) > 0)) {
			ask (people_in_park where (each.state != 2 and each.state != 3 and each.state != 1 and each.state != 4)) {
				if (have_mask) {
					if (flip(infected_rate * mask_rate)) {
						state <- 1;
						save_time <- nb_day;
...
species school {
	...
	action test {
		if (is_light and !is_workhour) {
			capture (susceptible inside self) where (each.end_point != nil and each.state != 4) as: people_in_school;
			do inf();
		} else if (is_light and is_workhour) {
	...
	action inf {
		if (nb_total > 0 and (people_in_school count (each.state = 2) > 0)) {
			ask (people_in_school where (each.state != 2 and each.state != 3 and each.state != 1 and each.state != 4)) {
				if (have_mask) {
					if (flip(infected_rate * mask_rate)) {
						state <- 1;
						save_time <- nb_day;
...
species supermarket {
	...
	action test {
		if (is_light and !is_workhour) {
			capture (susceptible inside self) where (each.end_point != nil and each.state != 4) as: people_in_supermarket;
			do inf();
		} else if (is_light and is_workhour) {
	...
	action inf {
		if (nb_total > 0 and (people_in_supermarket count (each.state = 2) > 0)) {
			ask (people_in_supermarket where (each.state != 2 and each.state != 3 and each.state != 1 and each.state != 4)) {
				if (have_mask) {
					if (flip(infected_rate * mask_rate)) {
						state <- 1;
						save_time <- nb_day;
...
species susceptible skills: [moving] {
	...
	bool death_or_not <- flip(death_or_survive);
	...
	reflex moving {
		if (nb_day != save_time) {
			if (((nb_day - save_time) mod keeptimeE = 0) and state = 1) {
				state <- 2;
				save_time <- nb_day;
			} else if (((nb_day - save_time) mod keeptimeI = 0) and state = 2 and death_or_not) {
				state <- 3;
			}else if (((nb_day - save_time) mod keeptimeI = 0) and state = 2 and !death_or_not){
				state <- 4;
			}
...
experiment "Run" {
	...
	parameter "Survive rate" var: death_or_survive min:0.0 max: 1.0;
	...
	output {
		...
		monitor "nb_of_D" value: susceptible count (each.state = 4);
	}
